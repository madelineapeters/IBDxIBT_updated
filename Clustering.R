...
for (p in 12){
  for (r in 1){
    start_wd = (paste("/Users/madeline/Desktop/Weis.lab/IBDxIBT", paste("para_set", p, sep="_"), paste("model_run", r, sep="_"), sep="/"))
    setwd(start_wd)
    
    clust.df = as.data.frame(matrix(nrow=20, ncol=length(seq)))
    names(clust.df) = c("gen1", "gen150", "gen300")
    
    for (z in seq) {
      offspring_map = read.csv(paste(getwd(), paste("paraset_",p,"_offspring_map_", z, ".csv", sep=""), sep="/"))
      fit = Mclust(offspring_map[,1:3], modelNames = c("EVI", "VVV", "VVI", "VII"))
      #models: EVI (diagonal, varying volume and shape), VVV (ellipoidal, varying volume, shape and orientation,
      # VVI (varying volume and shape), VII (spherical, unequal volume))
      ...
      uncert.plo = -plot(fit, what = c("uncertainty"), 
