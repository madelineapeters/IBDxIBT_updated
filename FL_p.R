...
x <- as.integer(args[1])
  ...
  for (r in 1:no_runs) {
    ...
    start_wd<-paste(base_wd,sub_wd, paste("para_set", x, sep="_"), paste("model_run", r, sep="_"), sep="/")
    setwd(start_wd)
     ...
     # }
      ...
      for (n in 1:no_neutral){
        ...
        if (i == 1){
          ...
          allele.df[((i/50)+1),n]<-dev_p
        }
        
      } #end loop over n
      
    } #end for loop over generations
    write.csv(allele.df, paste("FL.dev.p.", r, ".csv", sep=""))
  ...
  start_wd<-paste(base_wd, sub_wd, paste("para_set", x, sep="_"), sep="/")
  setwd(start_wd)
  
  runavg.df<-rowMeans(avg.df)
  run.avg.df<-as.data.frame(run.avg.df)
  write.csv(avg.df, paste("Sep.FL.p.", x, ".csv", sep=""))
  write.csv(run.avg.df, paste("Avg.FL.p.", x, ".csv", sep=""))
