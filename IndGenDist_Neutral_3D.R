...
for (s in 1:8){
  for (g in 1:length(g.list)){
    ...
    dist.mat = dist(cbind(Mydata$X_pos,Mydata$Y_pos,Mydata$FLday))
    ...
    write.csv(Mantel.obs,'Mantel.obs.3D.csv',row.names=FALSE)
    write.csv(Mantel.p,'Mantel.p.3D.csv',row.names=FALSE)
  }  
}
...
comment.out = function(){
  ...
  Mantel.obs = read.csv(paste(getwd(),'Mantel.obs.3D.csv',sep="/"))
  Mantel.obs.plot = gather(Mantel.obs,'paraset','correlation')
  Mantel.obs.plot$generation = g.list
  Mantel.p = read.csv(paste(getwd(),'Mantel.p.3D.csv',sep="/"))
